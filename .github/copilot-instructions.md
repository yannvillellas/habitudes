...
- **Layered architecture**: Each layer communicates only with the layer directly below. UI → ViewModel → Repository → Service. No skipping layers.
...
- Do not name shared widget directories `widgets/` (conflicts with Flutter SDK); use `ui/core/`.
...
- Repositories **must never be aware of each other**. Combine data from multiple repos in the view model or domain layer.
- Repository dependencies (services) are **private** members — UI must never bypass the repository to call a service.
...
- View = a `StatelessWidget` that takes a ViewModel as constructor parameter (`required this.viewModel`).
...
- Add use-cases only when: logic requires merging multiple repositories, is exceedingly complex, or is reused across ViewModels.
